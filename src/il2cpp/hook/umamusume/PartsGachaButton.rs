...
extern "C" fn InitializeJp(this: *mut Il2CppObject, executable: *mut Il2CppObject, card_type: i32, ticket_counter: *mut Il2CppObject, on_success: *mut Il2CppObject, is_only_one: bool, index: i32, is_small: bool, open_dialog_type: i32) {
    ...
    if this.is_null() || executable.is_null() {
        return;
    }
...
fn apply_gacha_button_config(this: *mut Il2CppObject, executable: *mut Il2CppObject) {
    ...
    let Some(gacha_overrides) = config.gacha_buttons_override.as_ref() else {
        return;
    };

    let button_config = select_button_config(executable, gacha_overrides);
    let Some((cfg, cfg_name)) = button_config else {
        return;
    };
...
fn apply_rect_offset(obj: *mut Il2CppObject, config: &UITextConfig) {
    ...
    let Some(rect_transform) = get_rect_transform(obj) else { 
        return;
    };
    let Some((get_pos_fn, set_pos_fn)) = get_position_functions(rect_transform) else { 
        return;
    };
